...
Implementierung: `*.py`-Module mit `TOOLS` + `HANDLERS` unter **konfigurierten Tool-Wurzeln** (`AGENT_TOOL_DIRS` oder Standard: `agent_tools` im Image + optional `AGENT_TOOLS_EXTRA_DIR`). Die Registry scannt **rekursiv**; im Image sind Module nach **Layer** gruppiert (`core/`, `knowledge/`, `external/`, `productivity/`, `domains/`). Kein Tool ist im HTTP-Core eingetragen. Secrets nur über **`.env`** / Env, siehe `docker/.env.example`.

**Begriffe:** Nur die **HTTP-API** des Agent-Layers ist **OpenAI-kompatibel** (Clients wie Open WebUI). Eure **Tools** sind registrierte Python-Module; die Einträge in `tools[]` nutzen dasselbe **JSON-Layout** wie Chat Completions (`type` / `function` / …), sind aber **eure** Tools — keine „OpenAI-Tools“.

**Stufenweise Erkundung (Pflichtmodell):** In `tools[]` an Ollama sind **alle** Tools immer nur **Katalog** (Name, Kurzbeschreibung, minimale `parameters`). **Kein** Domänen-Tool erhält sein volles JSON-Schema in `tools[]`. **(1) Kategorien:** **`list_tool_categories`** → je Kategorie **id**, **TOOL_LABEL**, **Kurzbeschreibung**, Tool-Anzahl. **(2) Tools pro Kategorie:** **`list_tools_in_category(category)`** → Namen + Kurzbeschreibungen in dieser Kategorie. **(3) Volles Spektrum nur nach Auswahl:** **`get_tool_help(tool_name)`** — die **Tool-Antwort** liefert **TOOL_DESCRIPTION** + **parameters** (volles Schema), dazu **`module_source`** (`file:…` zur definierenden `.py`, aus Registry-Metadaten im Speicher — **nicht** Postgres) und **`readable_via_read_tool`** / **`read_tool_hint`** (ob **`read_tool`** für dieses Modul überhaupt geht). Erst danach sinnvoller Aufruf des Domänen-Tools (Schema liegt in der Historie, nicht in `tools[]`). **`list_available_tools`** bleibt flache Namen+Kurztexte. **Router** steuert nur, **welche** Tools überhaupt in der gemergten Liste sind (Trigger in der letzten User-Message → Vereinigung der Kategorien + Introspection-Tools; kein Treffer → volle Registry, weiterhin nur Katalog). Zusätzliche Kategorien explizit: Header **`X-Agent-Router-Categories`** oder Body **`agent_router_categories`**. **`GET /v1/router/categories`**. **`AGENT_TOOLS_DENYLIST`**. Optional **`tool_prefetch`**. Module: **`TOOL_DOMAIN`**, **`TOOL_TRIGGERS`**; optional **`AGENT_TOOL_DOMAIN_ORDER`** (Reihenfolge). Details: `docker/.env.example`.
...
| [x] | `list_tool_categories` | `tool_help` | Router-Kategorien: id, TOOL_LABEL, Kurztext, Tool-Anzahl (→ `list_tools_in_category`). |
...
| [x] | `create_tool` | `core/tool_factory/create_tool.py` | Neues Extra-Tool (Quelltext oder Codegen via Ollama); `AGENT_CREATE_TOOL_ENABLED=true` + beschreibbares Extra-Verzeichnis. |
| [x] | `list_tools` | `core/tool_factory/list_tools.py` | Dateinamen (`.py`, eine Ebene) im beschreibbaren Tool-Modul-Ordner (`AGENT_TOOLS_EXTRA_DIR`). |
| [x] | `read_tool` | `core/tool_factory/read_tool.py` | Quelltext **nur** für `.py` unter **`AGENT_TOOLS_EXTRA_DIR`** (Extras); eingebaute Image-Tools (`openweather_*`, …) haben hier **keinen** Pfad — Schema per **`get_tool_help`**. |
| [x] | `update_tool` | `core/tool_factory/update_tool.py` | Patch (`old_string`/`new_string`); Ziel wie bei `read_tool`. Verwechslungen (`overwrite`, `TOOL_DESCRIPTION`, `source`) → klare JSON-Fehler. |
| [x] | `replace_tool` | `core/tool_factory/replace_tool.py` | Ganze Datei durch `source`; Ziel wie bei `read_tool`. |
| [x] | `rename_tool` | `core/tool_factory/rename_tool.py` | `.py`-Datei umbenennen (Reload). |
...
| [x] | `rag_search` | `rag` | Semantische Suche über ingestete Dokumente (pgvector); Ingest per `POST /v1/admin/rag/ingest`. |
...
| [x] | `outdoor_environment_snapshot` | `outdoor_snapshot` | Zeit (IANA-TZ) + optional OpenWeather bei `location` + grobes daylight/exposure-Hint; `tools_meta.domain=shared`. |

*(Frühere Tool-Namen `issue_secret_registration_otp` / `secret_storage_help` — durch Umbenennung ersetzt.)*

**Hilfe / „Welche Tools gibt es?“**

- In **`tools[]`** pro Request: nur **Kurz**-`TOOL_DESCRIPTION` + Katalog-Hinweis; **volle** **`parameters`** liefert **`get_tool_help(tool_name)`** in der Tool-Antwort.
...
- **Aufbau des Quelltexts:** Wie `docker/extra_tools/sample_echo.py`: Modul mit **`TOOLS`** (Liste im Chat-`tools[]`-Format), **`HANDLERS`** (Name → Callable), Handler-Funktionen mit `def foo(arguments: dict) -> str` und `return json.dumps(...)`. Jedes Element von **`TOOLS`** muss **`{\"type\": \"function\", \"function\": {\"name\": \"…\", \"TOOL_DESCRIPTION\": \"…\", \"parameters\": {...}}}`** sein — der **`name`** gehört **unter** `function`, nicht auf die oberste Ebene des Dicts (häufiger Codegen-Fehler; vor dem Schreiben prüft der Agent `validate_tool_registry_exports`). AST-Check verbietet u. a. `subprocess`, `eval`/`exec`, `os.system` — **kein vollständiger Sandbox**; nur in vertrauenswürdigen Umgebungen aktivieren (**`AGENT_API_KEY`** empfohlen).
- **Anderes Tool aus einem Extra-Plugin aufrufen:** `from app.plugin_invoke import invoke_registered_tool` — Argumente: **registrierter Tool-Funktionsname** (z. B. `openweather_forecast`) und ein **dict** wie im Chat. Rückgabe ist derselbe JSON-String wie bei einem normalen Tool-Call; bei Fehlern oft `{"ok": false, ...}`. Jeder Aufruf wird wie üblich in **`tool_invocations`** protokolliert. Tiefe begrenzt: **`AGENT_TOOL_CHAIN_MAX_DEPTH`** (Default 24). Rekursion (A ruft A) vermeiden. Alternative bleibt `from app.tools import run_tool` (identische Semantik).
...
```text
TOOLS: list[dict]          # Chat tools[] wire shape (type/function/…)
